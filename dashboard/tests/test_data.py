...
def _make_df():
    return pd.DataFrame([
        {
            'sensor_id': 'field_1_ble', 'timestamp': '2026-05-06T10:00:00Z',
            'temperature_c': 28.0, 'humidity_pct': 42.0, 'plant_stage': 'croissance',
            'stress_score': 20.0, 'alerts': [],
        },
        {
            'sensor_id': 'field_2_mais', 'timestamp': '2026-05-06T10:00:00Z',
...
def test_build_field_cards_no_data_for_missing_field():
    cards = build_field_cards(_make_df())
    field3 = next(c for c in cards if c['sensor_id'] == 'field_3_tournesol')
...
def test_build_field_cards_alerte_status():
    cards = build_field_cards(_make_df())
    field2 = next(c for c in cards if c['sensor_id'] == 'field_2_mais')
    assert field2['status'] == 'Alerte'


def test_build_field_cards_stage_translated():
    cards = build_field_cards(_make_df())
    field1 = next(c for c in cards if c['sensor_id'] == 'field_1_ble')
...
def test_build_detail_returns_empty_for_empty_df():
    assert build_detail(pd.DataFrame(), 'field_1_ble') == {}


def test_build_detail_returns_npk_values():
    detail = build_detail(_make_df_with_npk(), 'field_1_ble')
...
def test_build_report_table_no_yesterday_sets_flag():
    ...
    field1 = next(r for r in rows if r['sensor_id'] == 'field_1_ble')
